 ...
 * @package			phpshop
 * @subpackage		phpshop.app.views.order_items
 * @since			phpShop(tm)
 * @version			$Revision:$
...
<?php echo $form->create('OrderItem');?>
	<fieldset>
 		<legend><?php __('Edit OrderItem for Order: ' . $order_id);?></legend>
	<?php
		echo $form->input('id');
...
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('Delete', true), array('action'=>'delete', $form->value('OrderItem.id')), null, sprintf(__('Are you sure you want to delete # %s?', true), $form->value('OrderItem.id'))); ?></li>
