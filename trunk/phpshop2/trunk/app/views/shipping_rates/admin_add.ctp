...
<?php echo $form->create('ShippingRate');?>
	<fieldset>
 		<legend><?php __('Add ShippingRate');?></legend>
	<?php
		echo $form->input('country_id');
 		echo $form->input('zone_id', array('type'=>'select', 'options' => $zones, 'selected' => $this->data['ShippingRate']['zone_id'], 'empty' => ''));		echo $form->input('name');
